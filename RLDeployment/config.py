...
STATE_DIM = (2 * MAX_DEPLOYABLE_NODES) + 2 
...
BATCH_SIZE = 64             # 每次训练时从缓冲区采样的批次大小
TARGET_UPDATE_FREQ = 20     # 目标网络更新频率 (以 Episodes 为单位)
GRAD_CLIP_NORM = 1.0
...
# --- 训练过程控制 ---
MAX_EPISODES = 10000         # 最大训练回合数 (Episodes)
START_TRAIN_EPISODE = 50   # 在开始训练前收集的最小经验数量
MODEL_SAVE_FREQ = 20        # 模型权重保存频率 (以 Episodes 为单位)
